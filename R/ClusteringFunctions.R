...
###############################################################################
#' Peak clustering using global maximum greedy approach
#'
#' @description Alternative clustering method that prioritizes globally
#' strongest peaks first, then excludes their neighborhood before selecting
#' the next peak.
#'
#' @param tss.dt Data table with TSS positions and tags
#' @param peakDistance Minimum distance between peaks
#' @param localThreshold Local filtering threshold
#' @param extensionDistance Extension distance for cluster boundaries
...
.clusterByPeakMax <- function(tss.dt, peakDistance, localThreshold, extensionDistance) {
  ...
  #### - Global Maximum Greedy Peak Selection - ####

  # Create copy for peak finding
  ...
  peakMaxID <- vector("numeric", nrow(peakpos.dt))
  idx <- 0

  # Greedy peak selection: always pick the globally highest peak,
  # then remove all positions within peakDistance
  while (nrow(peakpos.dt) > 0) {
    idx <- idx + 1
    # Get position of current highest peak
    peakpos_i <- peakpos.dt[1, pos]

    # Save peak position
    peakMaxID[idx] <- peakpos_i

    # Remove all positions within peakDistance of this peak
    peakpos.dt <- peakpos.dt[pos < peakpos_i - peakDistance |
                              pos > peakpos_i + peakDistance, ]
  }

  # Trim to actual size and sort
  peakMaxID <- sort(peakMaxID[1:idx])
  ...
  # Run peakclu validation on peaks from peakcluMax
  ...
  peakID <- vapply(peakMaxNewID, function(x) {
    id <- 0
    temp <- tss.dt[x, pos]
    if(tss.dt[x, pos] == tss.dt[pos > temp - peakDistance &
                                 pos < temp + peakDistance, ][which(tags == max(tags)), pos][1]){
      id <- x
    } else {
      id <- 0
    }
  ...
  #### - End of Global Maximum Selection - ####

  ###############################################################################
  ## Local filtering (same as peakclu)
  ###############################################################################
  if(unique(tss.dt$strand) == "+"){
    localF <- sapply(peakID[peakID > 0], function(i){
      temp <- tss.dt[pos >= tss.dt$pos[i] & pos <= tss.dt$pos[i] + peakDistance, ]
  ...
  # Get full clustering data with quantile boundaries
  tss_clusters <- lapply(as.list(seq_len(clusters[, .N])), function(i) {
    start <- clusters[i, V1]
    end <- clusters[i, V2]
    cluster.data <- copied.dt[pos >= start & pos <= end, ]
